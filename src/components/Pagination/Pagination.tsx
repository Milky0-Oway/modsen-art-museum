...
const Pagination = ({ currentPage, totalPages=10, onPageChange }: PaginationProps): JSX.Element => {
    const [offset, setOffset] = useState(0);
    const pagesPerSet = 4;
    const pagesLength = Math.min(pagesPerSet, totalPages - offset);
    const pages = Array.from({ length: pagesLength }).map((_, i) => i + 1 + offset);
    ...
    return (
        <div className='pagination-wrapper'>
            ...
            <div className='page-numbers'>
                {pages.map((page) => (
                    <div className={`page-number ${page === currentPage ? 'selected' : ''}`} onClick={() => onPageChange(page)}>{page}</div>
                ))}
            </div>
